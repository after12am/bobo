...
class MarkovAgent {
    ...
    public function save($twitter) {
        ...
        if (!preg_match('/^([0-9]+)$/', $twitter['id_str'])) return false;
        if ($this->exist($twitter['id_str'])) return false;
        ...
        try {
            ...
            foreach ($rows as $row) $this->db->insert('markov', $row);
            $this->db->commit();
    ...
    public function text($hashtags = array()) {
        ...
        while ($add['lex3'] !== 'EOF') {
            ...
            // set to a 140-character limit
            if ($textLen + $addLen + $hashLen > 140) break;
    ...
    private function backup($text) {
        ...
        $backup = array();
        // backup URL
        preg_match_all("/https?:\/\/[-_.!~*\'()a-zA-Z0-9;\/?:\@&=+\$,%#]+/", $replaced_text, $m);
        if (count($m)) {
            foreach ($m[0] as $k => $v) {
                $_v = str_replace('/', '\/', $v);
